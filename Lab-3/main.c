...
int main(void) {
    ...
    //timer Stuff
    timer = startMonth * 2628288 + startDay * 86400;
    _enable_interrupt();
    _BIS_SR(GIE);
    initTimer_A();
    wheelSetup();
    ...
    while(1){
        ...
        //Go into edit mode with scroll wheel
            ...
            displayWheel(in_value); //Used for debug purposes

            //Modify day, will later implement for other time aspects

            if(in_value> 4082 || prev_in_value+50<in_value){
                timer += 86400;
                displayTime(timer);
            }
            else if(prev_in_value-50>in_value){
                timer -= 86400;
                displayTime(timer);
            }
...
//    int startDay = 17;
    ...
    output[0]=months[month][0];
    output[1]=months[month][1];
    output[2]=months[month][2];
    output[4]=dayStr[0];
    output[5]=dayStr[1];
    output[7]=hrStr[0];
    output[8]=hrStr[1];
